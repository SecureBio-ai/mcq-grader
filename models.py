...
REPLICATE_MODELS = ["llama2_70b", "llama2_13b", "llama2_7b", "gpt-2-xl", "gpt-2-large", "airoboros", "spicyboros", "falcon"]
ANTHROPIC_MODELS = ["claude2"]
AVAILABLE_MODELS = OPENAI_MODELS + REPLICATE_MODELS + ANTHROPIC_MODELS

REPLICATE_LINKS = {"llama2_70b": "meta/llama-2-70b-chat:35042c9a33ac8fd5e29e27fb3197f33aa483f72c2ce3b0b9d201155c7fd2a287"}
    ...
    for model_name in model_list:
        ...
        if model_name in OPENAI_MODELS:
            ...
            if not openai_api_token:
                raise Exception(f"OpenAI model {model_name} specified but no OpenAI API Token found in env var OPENAI_API_TOKEN")
            api_links[model_name] = "openai"
        if model_name in REPLICATE_MODELS:
            replicate_api_token = os.environ.get("REPLICATE_API_TOKEN")
            if not replicate_api_token:
                raise Exception(f"Replicate model {model_name} specified but no Replicate API Token found in env var REPLICATE_API_TOKEN")
        ...
        if model_name in ANTHROPIC_MODELS:
            ...
            if not anthropic_api_token:
                raise Exception(f"Anthropic model {model_name} specified but no Anthropic API Token found in env var ANTHROPIC_API_TOKEN")
